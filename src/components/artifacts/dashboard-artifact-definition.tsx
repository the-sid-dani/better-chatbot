...
import { BarChart3, Download, RefreshCw, Copy, Share2, FileText, TrendingUp } from "lucide-react";
import { DashboardArtifactMetadata } from "../../types/artifacts";

// Dashboard artifact definition
export const dashboardArtifactDefinition = new Artifact<"dashboard", DashboardArtifactMetadata>({
  kind: "dashboard",
  description: "Create comprehensive dashboards with multiple charts, metrics, and analysis",
  ...
  toolbar: [
    {
      ...
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: "user",
          content: "Please refresh the dashboard data with the latest information."
        });
      }
    },
    {
      ...
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: "user",
          content: "Add more charts to this dashboard with additional data analysis."
        });
      }
    },
    {
      ...
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: "user",
          content: "Generate a detailed report based on this dashboard's data and insights."
        });
      }
    },
    {
    ...
    },
    {
      ...
      onClick: ({ artifact, appendMessage }) => {
        appendMessage({
          role: "user",
          content: "How can I share this dashboard with my team and set up collaborative access?"
        });
      }
    }
  ],
    ...
    },
    {
      ...
      onClick: ({ artifact, appendMessage }) => {
        try {
          ...
          appendMessage({
            role: "user",
            content: "Create a similar dashboard with comparable structure and different sample data."
          });
        }
      }
    },
    {
      ...
      onClick: ({ artifact, appendMessage }) => {
        try {
          ...
          appendMessage({
            role: "user",
            content: "Convert this dashboard into a presentation format with key insights and narrative structure."
          });
        }
      }
    }
