  ...
  const messages: UIMessage[] = (thread?.messages ?? []).map((m) => {
    return {
      ...
      parts: m.parts.filter((part: any) => {
        // CLEANUP: Drop tool parts that saved an empty object input; Anthropic
        // rejects "{}" but we keep original payloads otherwise.
        if (
      ...
      const result = streamText({
        ...
        onFinish: async (result) => {
          ...
          try {
            ...
            // FINAL DEFENSE: In rare cases both the capture and the builder can
            // yield an empty parts array (e.g., provider returned neither text nor tools).
            // Prevent persisting an assistant message with no renderable parts.
            const finalResponseMessage: UIMessage =
              responseMessage.parts && responseMessage.parts.length > 0
                ? responseMessage
                : {
                    ...responseMessage,
                    // Always include at least a small text part so UI can render history
                    parts: [
                      {
                        type: "text" as const,
                        text: "(no output generated by the model – please retry or adjust your request)",
                      },
                    ],
                  };
            ...
            if ((responseMessage.parts?.length ?? 0) === 0) {
              logger.warn(
